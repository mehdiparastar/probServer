...
//                 if (thisScenario === scenarioName.GSMIdleMCI) {
//                   if (this.logStarted) {
//                     const location = await this.gpsDataRepo
//                       .createQueryBuilder()
//                       .where('ABS(TIMESTAMPDIFF(MICROSECOND, createdAt, :desiredCreatedAt)) <= 2000000') // One second has 1,000,000 microseconds
...
//                   }
//                 }
//                 if (thisScenario === scenarioName.ALLTechIdleMCI) {
//                   if (this.logStarted) {
//                     const location = await this.gpsDataRepo
//                       .createQueryBuilder()
//                       .where('ABS(TIMESTAMPDIFF(MICROSECOND, createdAt, :desiredCreatedAt)) <= 2000000') // One second has 1,000,000 microseconds
...
//                   }
//                 }
//                 if (thisScenario === scenarioName.GSMLongCallMCI) {
//                   if (this.logStarted) {
//                     const location = await this.gpsDataRepo
//                       .createQueryBuilder()
//                       .where('ABS(TIMESTAMPDIFF(MICROSECOND, createdAt, :desiredCreatedAt)) <= 2000000') // One second has 1,000,000 microseconds
...
//                   }
//                 }
//                 if (thisScenario === scenarioName.FTP_DL_TH_MCI) {
...
//                 if (thisScenario === scenarioName.WCDMAIdleMCI) {
//                   if (this.logStarted) {
//                     const location = await this.gpsDataRepo
//                       .createQueryBuilder()
//                       .where('ABS(TIMESTAMPDIFF(MICROSECOND, createdAt, :desiredCreatedAt)) <= 2000000') // One second has 1,000,000 microseconds
...
//                   }
//                 }
//                 if (thisScenario === scenarioName.WCDMALongCallMCI) {
//                   if (this.logStarted) {
//                     const location = await this.gpsDataRepo
//                       .createQueryBuilder()
//                       .where('ABS(TIMESTAMPDIFF(MICROSECOND, createdAt, :desiredCreatedAt)) <= 2000000') // One second has 1,000,000 microseconds
...
//                 if (thisScenario === scenarioName.LTEIdleMCI) {
//                   if (this.logStarted) {
//                     const location = await this.gpsDataRepo
//                       .createQueryBuilder()
//                       .where('ABS(TIMESTAMPDIFF(MICROSECOND, createdAt, :desiredCreatedAt)) <= 2000000') // One second has 1,000,000 microseconds
...
//                   // module in Idle mode
//                   const thisScenario = (await this.quectelsRepo.findOne({ where: { serialPortNumber: portNumber }, select: { activeScenario: true } })).activeScenario
//                   if (thisScenario === scenarioName.GSMLongCallMCI || thisScenario === scenarioName.WCDMALongCallMCI) {
...
//                 const thisScenario = (await this.quectelsRepo.findOne({ where: { serialPortNumber: portNumber }, select: { activeScenario: true } }))?.activeScenario
//                 if (thisScenario && thisScenario === scenarioName.FTP_DL_TH_MCI) {
//                   await sleep(300)
...
//                 let scenarios = [scenarioName.GSMIdleMCI, scenarioName.WCDMAIdleMCI, scenarioName.LTEIdleMCI, scenarioName.ALLTechIdleMCI, scenarioName.GSMLongCallMCI, scenarioName.WCDMALongCallMCI, scenarioName.FTP_DL_TH_MCI, scenarioName.FTP_UL_TH]

//                 scenarios = scenarios.filter(item => item !== scenarioName.FTP_DL_TH_MCI)
//                 const map = allEntries.reduce((p, c) => {
//                   if (c.IMEI === '866758042198099') {
//                     return {
//                       ...p,
//                       [c.IMEI]: scenarioName.FTP_DL_TH_MCI
//                     }
//                   }
//                   else if (c.modelName === 'EP06' && scenarios.includes(scenarioName.WCDMAIdleMCI)) {
//                     scenarios = scenarios.filter(item => item !== scenarioName.WCDMAIdleMCI)
//                     return {
//                       ...p,
//                       [c.IMEI]: scenarioName.WCDMAIdleMCI
//                     }
//                   }
//                   else if (c.modelName === 'EP06' && scenarios.includes(scenarioName.WCDMALongCallMCI)) {
//                     scenarios = scenarios.filter(item => item !== scenarioName.WCDMALongCallMCI)
//                     return {
//                       ...p,
//                       [c.IMEI]: scenarioName.WCDMALongCallMCI
//                     }
//                   }
//                   else {
...
//                     case scenarioName.ALLTechIdleMCI:
...
//                     case scenarioName.FTP_DL_TH_MCI:
//                       // this.ftpDLConnectionEstablishingProgressTime = (new Date()).getTime()
